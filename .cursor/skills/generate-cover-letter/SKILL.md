...
description: >-
  ...
  cover-letter best practices, profile and GitHub evidence, the writing-style
  prose rules at draft time, and a mandatory avoid-ai-writing double-check
  before handoff. Use when the user pastes a JD and asks for a cover letter,
  or runs /generate-cover-letter. For a full tailored CV use tailor-cv; for
  Toptal pitches use generate-toptal-pitch.
---
...
2. [sources/cover-letter-best-practices.md](../../../sources/cover-letter-best-practices.md) — structure, length, evidence rules; cite source IDs from [sources/references.json](../../../sources/references.json)
3. `profile/questionnaire.md` § Writing voice — personal overrides (first person OK; lead with role fit and one verified proof point; no generic enthusiasm closers)

**Evidence sources:**

- Base CV: `profile/base-cv-enhanced.md` (fallback: `profile/base-cv.md`)
- `profile/github-summary.md` — technical proof; respect `verified-from-github` vs `needs-your-confirmation`
- `profile/github-index.json` — grep for JD terms (commits, PR subjects) when the summary is thin on a required skill
- `profile/gap-report.md` (if present) — known gaps; never claim across them
...
### 4. Map evidence — pick one anchor

From `base-cv-enhanced.md`, `github-summary.md`, and (if needed) grep of
`github-index.json`, select:

- **One anchor example** for the body: a project or piece of work that hits
  the JD's core skill, with a constraint, a decision, what shipped, and a
  verified result. Depth beats breadth: one anchor told concretely, not three
  summarized.
- **One opening proof point**: the single strongest verified fact that maps
  to the JD.

**Never invent** employers, projects, metrics, or company facts. Do not use
`needs-your-confirmation` GitHub evidence without asking the user first.
Unsupported JD requirements go to the report as gaps.

### 5. Draft `cover-letter.md`
...
### 7. Write `cover-letter-report.md`

1. **Evidence map** — claim in letter | source (CV / github-summary / index / questionnaire) | verified?
2. **Company specifics** — fact used | where verified
3. **Gaps** — JD requirements not covered, honest list
4. **Best-practices checklist** — copy from [cover-letter-best-practices.md](../../../sources/cover-letter-best-practices.md); mark pass/fail
5. **Style pass** — table from [writing-style.md](../../../sources/writing-style.md) §5; mark pass/fail
6. **avoid-ai-writing audit** — findings from pass 1, fixes applied, pass 2 result
...
- Repeating CV bullets in paragraph form (the letter adds detail, never restates)
